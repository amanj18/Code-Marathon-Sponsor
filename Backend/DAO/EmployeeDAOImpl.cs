        ...
        public async Task<IEnumerable<Employee>> GetSponsorPaymentSummariesAsync()
        {
            ...
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                var query = @"
                    ...
                    FROM sponsor_sch.Sponsors s
                    LEFT JOIN sponsor_sch.Contracts c ON s.SponsorID = c.SponsorID
                    LEFT JOIN sponsor_sch.Payments p ON c.ContractID = p.ContractID
            ...
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                var query = @"
                    SELECT 
                        ...
                        m.Location,
                        COALESCE(SUM(p.AmountPaid), 0) AS TotalPayments
                    FROM sponsor_sch.Matches m
                    LEFT JOIN sponsor_sch.Contracts c ON m.MatchID = c.MatchID
                    LEFT JOIN sponsor_sch.Payments p ON c.ContractID = p.ContractID
        ...
        public async Task<IEnumerable<SponsorMatchCount>> GetSponsorMatchCountsAsync(int year)
        {
            ...
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                var query = @"
                    ...
                    FROM sponsor_sch.Sponsors s
                    LEFT JOIN sponsor_sch.Contracts c ON s.SponsorID = c.SponsorID
                    LEFT JOIN sponsor_sch.Matches m ON c.MatchID = m.MatchID
            ...
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var transaction = await connection.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        var contractExistsQuery = "SELECT COUNT(1) FROM sponsor_sch.Contracts WHERE ContractID = @ContractID";
                        ...
                        var insertPaymentQuery = @"
                            INSERT INTO sponsor_sch.Payments (ContractID, PaymentDate, AmountPaid, PaymentStatus)
